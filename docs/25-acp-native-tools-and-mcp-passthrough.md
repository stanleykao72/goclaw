...
- **ACP session MCP type** — ⚠️ on the target branch **`esmith/main`** this is the
  **old form**, not the rich struct an earlier draft assumed.
  `internal/providers/acp/types.go` declares:

  ```go
  type NewSessionRequest struct {
      Cwd        string   `json:"cwd"`
      McpServers []string `json:"mcpServers"`   // string slice, not objects
  }
  ```

  and `session.go` sends `McpServers: []string{}`. There is **no**
  `NewSessionMCPCfg{Name,URL,Command,Args,Env}` struct on `esmith/main` — that
  richer type exists only on the divergent `test/merge-main` lineage. **Therefore
  creating that struct (plus a `Headers` field) is part of the foundation work here,
  NOT a pre-existing asset.** `esmith/main` also carries `040b0f19` "Gemini ACP
  protocol fixes and multi-session architecture (#901)", so the session plumbing
  differs from earlier analyses — confirm exact shapes/line-refs against this branch
  (the §3 line numbers above and elsewhere were taken from `test/merge-main` and are
  indicative until re-pinned on `esmith/main`).
...
## 10. Branch & deployment note
**Target branch: `esmith/main`** (the active integration line; tip `b0c1f0df`).
Implementation cuts feature branches **from `esmith/main`** and PR-merges back into
it. Key baseline facts on this branch:

- ACP session uses the **old `McpServers []string`** form (see §3); the richer
  `NewSessionMCPCfg` type and the `--model` auto-inject (`ec0beb12`) live on the
  separate `test/merge-main` lineage and are **not** present here.
- `esmith/main` includes the multi-session ACP architecture (#901), so session
  lifecycle/plumbing differs from earlier `test/merge-main`-based notes.

Shipping to the VPS (`v3.12.0`) requires building from `esmith/main` and replacing
the binary, not just a config change. Memory Route A can ship before the
external-MCP work.
