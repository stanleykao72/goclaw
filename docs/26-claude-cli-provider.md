...
- **C1 (cred source-of-truth):** the lifted `ResolveExternalBridgeTools` replicates the inline merge ORDER (server APIKey → `headers["Authorization"]` `loop_mcp_user.go:139-141` → user APIKey override `:144-146` → `maps.Copy` user Headers/Env `:147-148`) and the `hasUserCreds → AcquireUser` decision EXACTLY. It does NOT claim full equivalence to `manager.resolveServerCredentials` (which adds a contextCreds tier unreachable on the bridge). **Test:** resolved headers map for `(odoo-prod, user)` is byte-identical between bridge helper and native loop; document the contextCreds gap.
  - **C1 implementation note (401 predicate widening, intentional):** the lift (`internal/mcp/external_tools.go` `BuildUserCredServerTools`) keys the 401-purge branch on the canonical `isUnauthorizedErr` (matches `unauthorized (401)`, `401 unauthorized`, `status code 401`, `http 401`) rather than the native loop's narrow `isUnauthorized401` (only `unauthorized (401)`). Because `getUserMCPTools` now delegates to the shared helper, the native loop's purge widens to all four forms too — a strictly-safer self-heal consistent with `BridgeTool.Execute`'s existing `isUnauthorizedErr` use. C1 byte-identical applies to the **headers map**, not the 401 predicate. Follow-up: add a native-loop test asserting a `status code 401` AcquireUser error triggers `DeleteUserCredentials`.
