...
// GroupMemoryCurationConfig configures the periodic, agent-driven curation of a
// LINE WORKS group's accumulated UNMENTIONED discussion into that group's vault
// LONGTERM.md. The curation runs through a normal agent turn (write_file tool ->
// MemoryInterceptor -> vault), NOT a direct channel-side vault write.
//
// Cadence is a cron expression evaluated in Timezone. The default restricts work
// to business hours (hourly 08:00-17:00, Asia/Taipei) so the sweep stays idle
// overnight. Only LINE WORKS groups are curated.
type GroupMemoryCurationConfig struct {
	Enabled  *bool  `json:"enabled,omitempty"`  // master on/off (default true)
	Cadence  string `json:"cadence,omitempty"`  // cron expression (default "0 8-17 * * *")
	Timezone string `json:"timezone,omitempty"` // IANA tz for the cron expr (default cron.default_timezone or "Asia/Taipei")
	// MinPending is the threshold: a group is only curated when its pending
	// message count is at or above this value (default 1, i.e. any pending).
	MinPending int `json:"min_pending,omitempty"`
	// KeepRecent is how many recent pending messages to retain after a SUCCESSFUL
	// curation; older ones are trimmed (default 40, mirrors compaction.keep_recent).
	KeepRecent int    `json:"keep_recent,omitempty"`
	Model      string `json:"model,omitempty"`    // curator model; empty = use agent's model
	Provider   string `json:"provider,omitempty"` // curator provider; empty = use agent's provider
	// DisabledGroups is a fallback per-group opt-out: history keys (LINE WORKS
	// channel/room IDs) listed here are skipped. See config_system.go loader and
	// the Stage 1 report for why this fallback is used instead of a dedicated
	// per-group config store.
	DisabledGroups []string `json:"disabled_groups,omitempty"`
}

// CurationDefaultCadence is the default business-hours cron expression: every
// hour on the hour from 08:00 to 17:00.
const CurationDefaultCadence = "0 8-17 * * *"

// CurationDefaultTimezone is the fallback timezone when neither curation.timezone
// nor cron.default_timezone is set.
const CurationDefaultTimezone = "Asia/Taipei"

// IsEnabled reports whether curation is enabled, defaulting to true when unset.
func (g *GroupMemoryCurationConfig) IsEnabled() bool {
	if g == nil {
		return true
	}
...
// IsGroupDisabled reports whether a specific group history key has been opted out
// via the disabled_groups fallback list.
func (g *GroupMemoryCurationConfig) IsGroupDisabled(historyKey string) bool {
	if g == nil {
		return false
	}
	for _, k := range g.DisabledGroups {
		if k == historyKey {
			return true
		}
	}
...
// EffectiveCadence returns the configured cron expression or the default.
func (g *GroupMemoryCurationConfig) EffectiveCadence() string {
	if g != nil && g.Cadence != "" {
		return g.Cadence
	}
	return CurationDefaultCadence
}

// EffectiveTimezone returns the configured timezone, falling back to the supplied
// cron default timezone, then to CurationDefaultTimezone.
func (g *GroupMemoryCurationConfig) EffectiveTimezone(cronDefaultTZ string) string {
	if g != nil && g.Timezone != "" {
		return g.Timezone
	}
	if cronDefaultTZ != "" {
		return cronDefaultTZ
	}
	return CurationDefaultTimezone
}

// EffectiveMinPending returns the pending threshold, defaulting to 1.
func (g *GroupMemoryCurationConfig) EffectiveMinPending() int {
	if g != nil && g.MinPending > 0 {
		return g.MinPending
	}
	return 1
}

// EffectiveKeepRecent returns how many recent pending messages to retain after a
// successful curation, defaulting to 40.
func (g *GroupMemoryCurationConfig) EffectiveKeepRecent() int {
	if g != nil && g.KeepRecent > 0 {
		return g.KeepRecent
	}
	return 40
}
...
type ChannelsConfig struct {
	...
	GroupCuration     *GroupMemoryCurationConfig `json:"group_curation,omitempty"`     // periodic agent-driven curation of LINE WORKS group pending into vault LONGTERM.md
}
